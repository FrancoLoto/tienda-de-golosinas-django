...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_email.send()
            # messages.success(request, 'Gracias por registrarte con nosotros. Te enviamos un correo de verificación para activar tu cuenta. Por favor, verifícalo.')
...
def forgotPassword(request):
    ...
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            # RESET PASSWORD
            current_site = get_current_site(request)
            mail_subject = 'Por favor, actualiza tu contraseña.'
            ...
            messages.success(request, 'Te enviamos un mail para actualizar la contraseña.')
            return redirect('login')
        else:
            messages.error(request, 'La cuenta ingresada no existe.')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Por favor, actualiza tu contraseña')
